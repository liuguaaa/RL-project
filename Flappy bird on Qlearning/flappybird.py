...
def showScore(screen, score, number_images):
	...
	width = 0
	for d in digits:
		width += number_images.get(d).get_width() #计算要显示分数所需要的宽度
	offset = (cfg.SCREENWIDTH - width) / 2
	for d in digits:
		screen.blit(number_images.get(d), (offset, cfg.SCREENHEIGHT*0.1)) #blit函数来绘制图像 (image,(x,y))
...
def main(mode, policy, agent, modelpath):
	...
	number_images = dict()
	for key, value in cfg.NUMBER_IMAGE_PATHS.items(): #遍历字典 其中包含多个键值对 键表示数字 值表示对应图像的路径
		number_images[key] = pygame.image.load(value).convert_alpha() #创建字典 键表示数字 键表示数字 值对应于图像
	# --the pipes
	pipe_images = dict()
	pipe_images['bottom'] = pygame.image.load(random.choice(list(cfg.PIPE_IMAGE_PATHS.values()))).convert_alpha() #选择管道图片
